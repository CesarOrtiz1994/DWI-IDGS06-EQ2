      ...
      <table>
        ...
        <tbody>
          <tr v-for="(libro, index) in listaDeLibros" :key="index">
            ...
            <td>{{ libro.correo }}</td>
            <td>{{ libro.nombres }}</td>
            <td>{{ libro.apellidoPaterno }}</td>
            <td>{{ libro.apellidoMaterno }}</td>
      ...
      <!-- Ventana modal -->
        ...
        <div class="modal-contenido">
          <div class="modal-header">
            <h2>{{ libroEditado ? 'Editar usuario' : 'Agregar nuevo usuario' }}</h2>
            <span @click="cerrarModal" class="cerrar-modal">&times;</span>
          </div>
              <div class="modal-body">
                <!-- Agrega los campos necesarios para agregar un nuevo libro -->
                <input v-model="nuevoLibro.nombreusuario" placeholder="Nombre usuario" />
                <input v-model="nuevoLibro.correo" placeholder="Correo" />
                <input v-model="nuevoLibro.nombres" placeholder="Nombres" />
                <input v-model="nuevoLibro.apellidoPaterno" placeholder="Apellido Paterno" />
                <input v-model="nuevoLibro.apellidoMaterno" placeholder="Apellido Materno" />
              </div>
